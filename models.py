...
class GNNStack(torch.nn.Module):
    ...
    def forward(self, data):
        ...
        for i in range(self.num_layers): # For each convolutional layer: conv -> ReLU -> dropout
            x = self.convs[i](x, edge_index)
            x = F.relu(x)
            x = F.dropout(x, p=self.dropout, training=self.training)
        if self.task == 'graph' and batch is not None: # If performing graph classification, aggregate node features
            x = pyg_nn.global_max_pool(x, batch) # Use global mean pooling (you can also try global max pooling)
...
class GraphSage(pyg_nn.MessagePassing):
    ...
    def forward(self, x, edge_index):
        ...
        neigh_agg = self.propagate(edge_index, x=(x,x))
        lin_out = self.lin(x)
        agg_out = self.agg_lin(neigh_agg)
        out = lin_out + agg_out
...
class GAT(pyg_nn.MessagePassing):
    ...
    def message(self, edge_index_i, x_i, x_j, size_i):
        ...
        alpha = pyg_utils.softmax(alpha, edge_index_i, ptr=None, num_nodes=size_i)
